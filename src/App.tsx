...
type RegistroRelatorioExcel = Vaga & {
  dataAdmissao?: string;
};
...
function AppAdministrativo() {
  ...
  function exportarRelatorioExcel() {
    const registros: RegistroRelatorioExcel[] = [
      ...vagas,
      ...admitidos,
    ];
    ...
    const totais =
      registros.reduce(
        (acc, vaga) => {
          ...
          acc.demanda += quantidade;
          acc.admissoes += admissoes;
          acc.pendentes += Math.max(
            0,
    ...
    const linhasTabela =
      Array.from(grupos.entries())
        .map(([unidade, lista]) => {
          const linhas =
            lista
              .map((vaga) => {
                ...
                return `
                  <tr>
                    <td>${escaparCelulaExcel(vaga.tipo || "OPERAC.")}</td>
                    <td>${escaparCelulaExcel(vaga.cargo || "")}</td>
                    <td>${escaparCelulaExcel(vaga.setor || "")}</td>
                    <td>${quantidade}</td>
                    <td>${escaparCelulaExcel(vaga.motivo || "")}</td>
                    <td>${escaparCelulaExcel(vaga.data || "03/07/2026")}</td>
                    <td>${admissoes}</td>
                    <td>${escaparCelulaExcel(admissoes > 0 ? vaga.dataAdmissao || "" : "")}</td>
                    <td>0</td>
                    <td>${pendentes}</td>
                  </tr>
                `;
          ...
          return `
            <tr class="unidade">
              <td colspan="10">UNIDADE ${escaparCelulaExcel(unidade)}</td>
            </tr>
            ${linhas}
            <tr class="total">
              <td colspan="3">UNIDADE ${escaparCelulaExcel(unidade)} - TOTAL</td>
              <td>${totalUnidade.vagas}</td>
              <td></td>
              <td></td>
              <td>${totalUnidade.admissoes}</td>
              <td></td>
              <td></td>
              <td>${totalUnidade.pendentes}</td>
            </tr>
          `;
        })
        .join("");
      ...
      <html>
        <head>
          ...
          <style>
            ...
            th { background: #0057b8; color: #ffffff; font-weight: 700; }
            th, td { border: 1px solid #9fb4cf; padding: 6px; font-size: 12px; }
            .titulo { background: #0057b8; color: #ffffff; font-size: 18px; font-weight: 700; text-align: center; }
            .subtitulo { background: #0057b8; color: #ffffff; font-size: 11px; text-align: center; }
            .resumo-numero { color: #0057b8; font-size: 18px; font-weight: 700; text-align: center; }
            .resumo-label { background: #0057b8; color: #ffffff; font-weight: 700; }
            .unidade { background: #0057b8; color: #ffffff; font-weight: 700; text-align: center; }
            .total { background: #eaf3ff; color: #003b82; font-weight: 700; text-align: center; }
          </style>
        </head>
        <body>
          <table>
            <tr>
              <td colspan="10" class="titulo">RELATÓRIO SEMANAL DE RECRUTAMENTO E SELEÇÃO</td>
            </tr>
            <tr>
              <td colspan="10" class="subtitulo">Relatório gerado automaticamente pelo RH | Tatyana Travassos - Coord. R&amp;S</td>
            </tr>
            <tr>
              <td class="resumo-numero">${totais.demanda}</td>
              <td class="resumo-label" colspan="2">DEMANDA ACUMULADA:</td>
              <td colspan="7">Total de vagas sob gestão.</td>
            </tr>
            <tr>
              <td class="resumo-numero">${totais.admissoes}</td>
              <td class="resumo-label" colspan="2">CONTRATAÇÕES EFETIVADAS:</td>
              <td colspan="7">Total de admissões realizadas no período.</td>
            </tr>
            <tr>
              <td class="resumo-numero">${totais.pendentes}</td>
              <td class="resumo-label" colspan="2">VAGAS PENDENTES:</td>
              <td colspan="7">Saldo de vagas ainda em processo de seleção.</td>
            </tr>
            <tr>
              <th>TIPO</th>
              <th>CARGO</th>
    ...
    baixarArquivoExcel(
      "relatorio-recrutamento-selecao.xls",
      html
    );
  }
...
function escaparCelulaExcel(valor: unknown) {
  return String(valor ?? "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
